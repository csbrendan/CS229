...
def train_model_with_alt_freezing(cfg_path, ckpt_path, real_dir, fake_dir, output_dir, total_epochs=10):
    ...
    batch_size = 4
    num_workers = 4
    train_dataset = DeepfakeDataset(real_dir, fake_dir, clip_size=32, max_videos=40) # quick for test 4/2
    val_dataset = DeepfakeDataset(real_dir, fake_dir, clip_size=32, max_videos=20)
    ...
    for epoch in range(1, total_epochs + 1):
        ...
        if val_metrics['accuracy'] > best_val_acc:
            ...
            model_save_path = os.path.join(output_dir, f'best_altfreeze_model_acc_{best_val_acc:.4f}.pth')
            torch.save({
                'epoch': epoch,
