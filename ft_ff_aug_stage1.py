...
class DeepfakeDataset(Dataset):
    def __init__(self, real_dir, fake_dir, clip_size=16, max_videos=200, save_samples=False, sample_dir=None):
        ...
        real_videos = [os.path.join(real_dir, f) for f in os.listdir(real_dir) if f.endswith('.mp4') and (f.startswith('8') or f.startswith('9'))][:max_videos//2]
        fake_videos = [os.path.join(fake_dir, f) for f in os.listdir(fake_dir) if f.endswith('.mp4') and (f.startswith('8') or f.startswith('9'))][:max_videos//2]
    ...
    def process_frame(self, frame, video_name=None, frame_idx=None):
        """Process frame with augmentations and optionally save"""

        if self.save_samples and video_name and frame_idx < 5:
            orig_save_path = os.path.join(self.sample_dir, f"{video_name}_frame_{frame_idx}_original.jpg")
            cv2.imwrite(orig_save_path, cv2.cvtColor(frame, cv2.COLOR_RGB2BGR))

        frame = self.augmenter._apply_quality_effects(frame)
        #frame = self.augmenter._apply_screen_lighting(frame)
        #frame = self.augmenter._apply_lighting_variations(frame)

        if self.save_samples and video_name and frame_idx < 5:
            save_path = os.path.join(self.sample_dir, f"{video_name}_frame_{frame_idx}.jpg")
...
def train_model(cfg_path, ckpt_path, real_dir, fake_dir, output_dir, num_epochs=10):
    cfg.init_with_yaml()
    ...
    train_dataset = DeepfakeDataset(real_dir, fake_dir, clip_size=cfg.clip_size, max_videos=200, save_samples=True, sample_dir=sample_dir)
    val_dataset = DeepfakeDataset(real_dir, fake_dir, clip_size=cfg.clip_size, max_videos=40, save_samples=False)
    ...
    for epoch in range(num_epochs):
        ...
        if val_metrics['accuracy'] > best_val_acc:
            ...
            torch.save(model.state_dict(), os.path.join(output_dir, 'stage_1_best_model.pth'))
            print(f"New best model saved with accuracy: {best_val_acc:.4f}")

        if epoch % 2 == 0:
            checkpoint_path = os.path.join(output_dir, f'stage1_checkpoint_epoch_{epoch}.pth')
            torch.save({
                'epoch': epoch,
