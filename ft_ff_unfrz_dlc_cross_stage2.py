...
def train_model(cfg_path, ckpt_path, real_dir, fake_dir, output_dir, num_epochs=20):
    ...
    for epoch in range(num_epochs):
        ...
        if val_metrics['accuracy'] > best_val_acc:
            ...
            torch.save(model.state_dict(), os.path.join(output_dir, 'stage_2_cross_best_model.pth'))
            print(f"New best model saved with accuracy: {best_val_acc:.4f}")

        if epoch % 2 == 0:
            checkpoint_path = os.path.join(output_dir, f'stage2_cross_checkpoint_epoch_{epoch}.pth')
            torch.save({
                'epoch': epoch,
